...
The specification is completely customizable using the web app workspace. The existing Hipster Stack UI for configuring the specification called `The Constituter` will be refactored to accommodate for the new direction we are taking.  
...
`The Constituter` is the webapp UI that is used to customize the configurations of the `Ontology` that the generator uses to initialize the `Ordinary Object` using the cli tool. It is purpose built to work with The Maximal Template™ Domain Library.  
...
- `Ontologies` are normalized specifications
  ...
  The top layer of `The Constituter` configuration UI allows the user to choose from 9 defaults that are starting points based standardized SaaS recipes
...
- The presentation layer is composed of pure UI block components
...
- Presentation blocks are composed of UI primitives
...
- Logic is orchestrated in workflows
  ...
  The next layer allows the user to configure the normalized workflows in each feature component by selecting/deselecting workflows from a list of all the workflows from The Maximal Template™
   ...
   - Workflows are composed of server operations and helpers
...
- Custom Inputs: If you build a custom input component styled with Tailwind, ensure its TypeScript props extend `React.InputHTMLAttributes<HTMLInputElement>`.
