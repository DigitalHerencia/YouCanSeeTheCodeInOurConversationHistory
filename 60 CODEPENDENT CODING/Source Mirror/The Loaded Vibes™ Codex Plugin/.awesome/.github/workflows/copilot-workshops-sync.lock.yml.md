---
title: 'The Loaded Vibes™ Codex Plugin\.awesome\.github\workflows\copilot-workshops-sync.lock.yml'
type: source-document
scope: project
project: 'Codependent Coding'
domain: source
artifact: 'The Loaded Vibes™ Codex Plugin\.awesome\.github\workflows\copilot-workshops-sync.lock.yml'
kind: source-document
namespace: 'codependentcoding.source.the-loaded-vibes-codex-plugin.awesome.github.workflows.copilot-workshops-sync.lock.yml'
status: active
authority: reference
parent:
depends_on: []
...
created: 2026-08-18
updated: 2026-08-18
source_path: 'The Loaded Vibes™ Codex Plugin\.awesome\.github\workflows\copilot-workshops-sync.lock.yml'
source_file: 'copilot-workshops-sync.lock.yml'
source_sha256: '39e5a3e81bf2e5cbbc5b244a6e67952ab9e7f70bb3fe008f1842c279fda1fe30'
generated: true
---
...
> Original path: `The Loaded Vibes™ Codex Plugin\.awesome\.github\workflows\copilot-workshops-sync.lock.yml`
> SHA-256: `39e5a3e81bf2e5cbbc5b244a6e67952ab9e7f70bb3fe008f1842c279fda1fe30`

```yaml
# gh-aw-metadata: {"schema_version":"v4","frontmatter_hash":"108ab49b9f7ff64384b1684e4dbb96b2ed20248f7ce44aadd105412ad344d54f","body_hash":"9bd5e98ebc05ca6910b0b4c105429140bf9548773bd210f42fde28c823505b4f","compiler_version":"v0.84.3","strict":true,"agent_id":"copilot","engine_versions":{"copilot":"1.0.77"}}
...
#
# Weekly check for updates to the Copilot Workshops source repo (github-samples/copilot-workshops). Opens a PR to keep the Learning Hub mirror aligned when substantive upstream course changes are detected.
#
# Secrets used:
...
on:
  schedule:
    - cron: "39 13 * * 4"  # Friendly format: weekly (scattered)
  workflow_dispatch:
    inputs:
      aw_context:
        default: ""
...
run-name: "Copilot Workshops Content Sync"
...
jobs:
  activation:
    ...
    steps:
      - name: Setup Scripts
        ...
        env:
          GH_AW_SETUP_WORKFLOW_NAME: "Copilot Workshops Content Sync"
          GH_AW_CURRENT_WORKFLOW_REF: ${{ github.repository }}/.github/workflows/copilot-workshops-sync.lock.yml@${{ github.ref }}
      ...
      - name: Generate agentic run info
        ...
        env:
          ...
          GH_AW_INFO_WORKFLOW_NAME: "Copilot Workshops Content Sync"
      ...
      - name: Restore daily AIC usage cache
        ...
        with:
          key: agentic-workflow-usage-copilotworkshopssync-${{ github.run_id }}
          restore-keys: agentic-workflow-usage-copilotworkshopssync-
      ...
      - name: Check daily workflow token guardrail
        ...
        uses: actions/github-script@3a2844b7e9c422d3c10d287c895573f7108da1b3 # v9.0.0
        env:
          GH_AW_WORKFLOW_NAME: "Copilot Workshops Content Sync"
          GH_AW_WORKFLOW_ID: "copilot-workshops-sync"
      ...
      - name: Check workflow lock file
        ...
        uses: actions/github-script@3a2844b7e9c422d3c10d287c895573f7108da1b3 # v9.0.0
        env:
          GH_AW_WORKFLOW_FILE: "copilot-workshops-sync.lock.yml"
      ...
      - name: Create prompt with built-in context
        ...
        run: |
          ...
          {
          cat << 'GH_AW_PROMPT_6dc1c0342a17500c_EOF'
          <system>
          GH_AW_PROMPT_6dc1c0342a17500c_EOF
          ...
          {{#runtime-import .github/workflows/copilot-workshops-sync.md}}
          GH_AW_PROMPT_6dc1c0342a17500c_EOF
          } > "$GH_AW_PROMPT"
  ...
  agent:
    ...
    env:
      ...
      GH_AW_RUNTIME_FEATURES: ${{ vars.GH_AW_RUNTIME_FEATURES }}
      GH_AW_WORKFLOW_ID_SANITIZED: copilotworkshopssync
    outputs:
    ...
    steps:
      ...
      - name: Generate Safe Outputs Config
        run: |
          ...
          cat > "${RUNNER_TEMP}/gh-aw/safeoutputs/config.json" << 'GH_AW_SAFE_OUTPUTS_CONFIG_68f4c1d3584c6c11_EOF'
          ...
          GH_AW_SAFE_OUTPUTS_CONFIG_68f4c1d3584c6c11_EOF
      - name: Generate Safe Outputs Tools
        env:
          GH_AW_TOOLS_META_JSON: |
            {
              "description_suffixes": {
                "create_pull_request": " CONSTRAINTS: Maximum 1 pull request(s) can be created. Title will be prefixed with \"[bot] \". Labels [\"automated-update\" \"learning-hub\" \"copilot-workshops\"] will be automatically added."
              },
              "repo_params": {},
              "dynamic_tools": []
            }
  ...
  conclusion:
    ...
    permissions:
      ...
      pull-requests: write
    concurrency:
      group: "gh-aw-conclusion-copilot-workshops-sync"
    ...
    steps:
      ...
      - name: Process no-op messages
        ...
        env:
          ...
          GH_AW_NOOP_MAX: "1"
          GH_AW_WORKFLOW_NAME: "Copilot Workshops Content Sync"
          GH_AW_WORKFLOW_SOURCE_URL: "${{ github.server_url }}/${{ github.repository }}/blob/${{ github.ref_name }}/.github/workflows/copilot-workshops-sync.md"
          GH_AW_RUN_URL: ${{ github.server_url }}/${{ github.repository }}/actions/runs/${{ github.run_id }}
  ...
  detection:
    ...
    steps:
      ...
      - name: Setup threat detection
        ...
        uses: actions/github-script@3a2844b7e9c422d3c10d287c895573f7108da1b3 # v9.0.0
        env:
          WORKFLOW_NAME: "Copilot Workshops Content Sync"
          WORKFLOW_DESCRIPTION: "Weekly check for updates to the Copilot Workshops source repo (github-samples/copilot-workshops). Opens a PR to keep the Learning Hub mirror aligned when substantive upstream course changes are detected."
          HAS_PATCH: ${{ needs.agent.outputs.has_patch }}
  ...
  safe_outputs:
    ...
    env:
      ...
      GH_AW_AIC: ${{ needs.agent.outputs.aic }}
      GH_AW_AMBIENT_CONTEXT: ${{ needs.agent.outputs.ambient_context }}
      GH_AW_CALLER_WORKFLOW_ID: "${{ github.repository }}/copilot-workshops-sync"
      GH_AW_DETECTION_CONCLUSION: ${{ needs.detection.outputs.detection_conclusion }}
      GH_AW_DETECTION_REASON: ${{ needs.detection.outputs.detection_reason }}
      GH_AW_EFFECTIVE_TOKENS: ${{ needs.agent.outputs.effective_tokens }}
      GH_AW_ENGINE_ID: "copilot"
