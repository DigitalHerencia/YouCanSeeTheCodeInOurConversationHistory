...
- Underlying Obsidian Markdown table editing and save persistence were verified on a disposable table: the cell changed from `original` to `updated` and read back after save. The Table Editor formatter command itself failed on the disposable fixture and remains unverified.
