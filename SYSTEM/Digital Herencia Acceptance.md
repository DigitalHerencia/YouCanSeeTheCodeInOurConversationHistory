...
- Hearth Projects now exposes direct Project and Task launchers plus Full Task Board access; live dashboard inspection confirmed all three controls render alongside the Active Projects view.
...
- Note Refactor selection extraction created a note containing the selected block through its live modal. Both selection-replace and split-from-cursor modes left the source note unchanged after modal completion, so source replacement/link persistence remains unverified.
